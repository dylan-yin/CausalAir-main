...
🎯 **离散因果发现**: 使用 Hard Gumbel-Softmax 采样生成离散的 0/1 因果矩阵，实现可解释的因果关系学习

🔥 **双层因果建模**: 
- 站点级因果矩阵 [N, N]：捕捉站点间的空间依赖关系
- 变量级因果矩阵 [N, 7, 56]：建模空气质量特征与气象特征之间的因果关系

⚡ **高效梯度传播**: 采用直通估计器 (Straight-Through Estimator) 实现离散采样的可微优化

📊 **可解释性强**: 学习到的因果矩阵清晰展示变量间的因果依赖关系

🎚️ **温度退火策略**: 支持线性和指数衰减的温度调度，在训练过程中逐步增强离散化程度
